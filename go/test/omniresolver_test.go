...
import (
	...
	omni "github.com/voxgig-sdk/waifu-pics-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WaifuPicsError); is {
				out := map[string]any{
					"name":    "WaifuPicsError",
...
func omniCtx(arg any, client *sdk.WaifuPicsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WaifuPicsSDK); is {
			client = live
