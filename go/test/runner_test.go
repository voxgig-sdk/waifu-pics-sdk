...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("WAIFUPICS_TEST_LIVE") == "TRUE" ||
		os.Getenv("WAIFUPICS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("WAIFUPICS_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.WaifuPicsSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.WaifuPicsError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.WaifuPicsSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.WaifuPicsError{Msg: msg, Code: code}
}
