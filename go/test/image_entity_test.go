...
func TestImageEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Image(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Image(nil).Stream("list", nil, nil) {
...
func imageBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("WAIFU_PICS_TEST_IMAGE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["WAIFU_PICS_TEST_IMAGE_ENTID"])
