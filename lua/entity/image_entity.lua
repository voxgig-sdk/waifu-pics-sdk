-- WaifuPics SDK Image entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ImageEntity = {}
ImageEntity.__index = ImageEntity


function ImageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ImageEntity)
  self._name = "image"
  self._client = client
...
function ImageEntity:get_name()
  return self._name
end


function ImageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ImageEntity.new(self._client, opts)
end
...
function ImageEntity:mark_deleted()
  self._deleted = true
end


function ImageEntity:deleted()
  return true == self._deleted
end


function ImageEntity:data_set(args)
  if args ~= nil then
...
function ImageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ImageEntity:match_set(args)
  if args ~= nil then
...
function ImageEntity:match_get()
...
function ImageEntity:stream(action, args, callopts)
...
function ImageEntity:_run_op(ctx, post_done)
  local utility = self._utility
