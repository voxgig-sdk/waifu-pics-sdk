...
MCP server exposing the WaifuPics SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `waifu-pics_list` | `entity`, optional `query` map | First page of records as JSON |
| `waifu-pics_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `waifu-pics_list` and
`waifu-pics_load` tools become available in new sessions.
