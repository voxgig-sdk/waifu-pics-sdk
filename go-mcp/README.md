...
[MCP](https://modelcontextprotocol.io) server exposing the WaifuPics SDK as
two agent tools — `waifu-pics_list` and `waifu-pics_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/waifu-pics-mcp)
make build
...
claude mcp add --scope user waifu-pics \
  -- /absolute/path/to/waifu-pics-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./waifu-pics-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// waifu-pics_list: first page of records
{ "entity": "image" }
{ "entity": "image", "query": { } }

// waifu-pics_load: one record by id
{ "entity": "image", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/waifu-pics-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user waifu-pics \
     -- "$PWD"/dist/*/waifu-pics-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `waifu-pics_list` and `waifu-pics_load` tools now appear
   in new sessions. Ask the agent to *"list image using waifu-pics"*
   and it calls `waifu-pics_list` with `{"entity":"image"}`.
...
| Tool | Args | Returns |
|------|------|---------|
| `waifu-pics_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `waifu-pics_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WAIFU_PICS_APIKEY` | API key sent with every request. |
| `WAIFU_PICS_BASE` | Optional override of the API base URL. |
...
```sh
./waifu-pics-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"waifu-pics_load","arguments":{"entity":"image","query":{"id":1}}}}'
```
