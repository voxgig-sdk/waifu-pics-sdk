...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = WaifuPicsOmni.make_runner(TEST_JSON_FILE, WaifuPicsSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : WaifuPicsOmni.errify(err)
    end
