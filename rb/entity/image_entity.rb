...
class ImageEntity
  ...
  def make
    opts = @_entopts.dup
    ImageEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = WaifuPicsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = WaifuPicsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.Image.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Image, Hash] the loaded Image; raises WaifuPicsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = WaifuPicsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
