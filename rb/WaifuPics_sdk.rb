...
require_relative 'WaifuPics_types'
...
class WaifuPicsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = WaifuPicsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = WaifuPicsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, WaifuPicsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    WaifuPicsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = WaifuPicsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = WaifuPicsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = WaifuPicsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => WaifuPicsError.new(
        "#{op}_allow",
        "WaifuPicsSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = WaifuPicsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = WaifuPicsError.new(
        "graphql_error", "WaifuPicsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
