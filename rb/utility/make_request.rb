...
module WaifuPicsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WaifuPicsResponse.new({})
    ...
    if fetch_err
      ...
      response = WaifuPicsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WaifuPicsResponse.new(fetched)
    else
