# WaifuPics SDK test runner

from __future__ import annotations
import os
import json

from waifupics_sdk.utility.voxgig_struct import voxgig_struct as vs


class WaifuPicsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WaifuPicsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WaifuPicsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WaifuPicsTestRunner.getenv("WAIFU_PICS_TEST_LIVE")
        override = WaifuPicsTestRunner.getenv("WAIFU_PICS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WaifuPicsTestRunner.getenv(key)
        ...
        explain = WaifuPicsTestRunner.getenv("WAIFU_PICS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["WAIFU_PICS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if WaifuPicsTestRunner._test_control is not None:
            return WaifuPicsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WaifuPicsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WaifuPicsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WaifuPicsTestRunner.load_test_control()
...
def load_env_local():
    WaifuPicsTestRunner.load_env_local()


def env_override(m):
    return WaifuPicsTestRunner.env_override(m)


def entity_data(v):
    return WaifuPicsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WaifuPicsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WaifuPicsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WaifuPicsTestRunner.load_test_control()


def live_client_options():
    return WaifuPicsTestRunner.live_client_options()


def live_delay_ms():
    return WaifuPicsTestRunner.live_delay_ms()
