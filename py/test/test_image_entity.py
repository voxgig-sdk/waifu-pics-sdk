...
class TestImageEntity:
    ...
    def test_should_stream(self):
        ...
        base = WaifuPicsSDK.test(seed, None)
        seen = list(base.Image(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = WaifuPicsSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Image(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _image_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "image." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set WAIFU_PICS_TEST_IMAGE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        image_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.image")))
        image_ref01_data = None
        if len(image_ref01_data_raw) > 0:
...
def _image_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/image/ImageTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("WAIFU_PICS_TEST_IMAGE_ENTID"))
    ...
    if env.get("WAIFU_PICS_TEST_LIVE") == "TRUE":
        ...
        client = WaifuPicsSDK(helpers.to_map(merged_opts))

    _live = env.get("WAIFU_PICS_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("WAIFU_PICS_TEST_EXPLAIN") == "TRUE",
        "live": _live,
