    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set WAIFUPICS_TEST_IMAGE_ENTID JSON to run live")
        client = setup["client"]
...
def _image_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("WAIFUPICS_TEST_IMAGE_ENTID"))
    ...
    _live = env.get("WAIFUPICS_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("WAIFUPICS_TEST_EXPLAIN") == "TRUE",
        "live": _live,
