...
class FtCtrl extends WaifuPicsControl
{
...
class FtClient
{
    ...
    private WaifuPicsUtility $utility;

    public function __construct(WaifuPicsUtility $utility, array $options)
    {
    ...
    public function get_utility(): WaifuPicsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WaifuPicsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WaifuPicsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (WaifuPicsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WaifuPicsConfig::make_feature($name);
    ...
    private static function buildUrl(WaifuPicsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WaifuPicsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WaifuPicsResponse($fetched);
        }
    ...
    private function fail(WaifuPicsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WaifuPicsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
