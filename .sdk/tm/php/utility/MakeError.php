...
class WaifuPicsMakeError
{
    public static function call(?WaifuPicsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new WaifuPicsContext([], null);
        }
        $op = $ctx->op ?? new WaifuPicsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new WaifuPicsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof WaifuPicsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new WaifuPicsError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof WaifuPicsError) {
