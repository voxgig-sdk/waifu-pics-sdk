...
from projectname_sdk.core.control import WaifuPicsControl
...
class WaifuPicsContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, WaifuPicsSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, WaifuPicsResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, WaifuPicsResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return WaifuPicsError(code, msg, self)
