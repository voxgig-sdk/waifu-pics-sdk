...
AQL-driven command-line client **and** interactive REPL for the WaifuPics
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/waifu-pics-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/waifu-pics-cli
   ```
...
Run with no arguments to open a REPL (prompt `waifu-pics>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `image`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/waifu-pics-cli`. |
...
(`./waifu-pics-cli <expr>`) and interactively (the REPL), and expressions compose the
