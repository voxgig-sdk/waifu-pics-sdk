// Command WaifuPics-cli is a CLI + REPL for the WaifuPics SDK,
...
const prompt = "waifu-pics"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "image"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      WAIFU_PICS_APIKEY (api key), WAIFU_PICS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: WAIFU_PICS_APIKEY carries the API key and
	// WAIFU_PICS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("WAIFU_PICS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("WAIFU_PICS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewWaifuPicsSDK(opts)
