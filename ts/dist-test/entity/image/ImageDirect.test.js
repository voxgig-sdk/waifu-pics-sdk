...
(0, node_test_1.describe)('ImageDirect', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when WAIFU_PICS_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('WAIFU_PICS_TEST_LIVE'));
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.WaifuPicsSDK({
    ...
    (0, node_test_1.test)('direct-load-image', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-image', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.WAIFU_PICS_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.WaifuPicsSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['WAIFU_PICS_TEST_IMAGE_ENTID'];
    ...
    const client = new __1.WaifuPicsSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=ImageDirect.test.js.map