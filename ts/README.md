...
`client.Image()` — each with a small set of operations (`load`)
...
await entity.load({ category: 'example_category', type: 'example_type' })
...
| `tester(testopts?, sdkopts?)` | `WaifuPicsSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `WaifuPicsSDK.test(testopts?, sdkopts?)` | `WaifuPicsSDK` | Create a test-mode client. |
...
| `client` | `client(): WaifuPicsSDK` | Return the parent SDK client. |
...
```ts
const image = await client.Image().load({ category: 'category', type: 'type' })
```
...
waifu-pics/
├── src/
│   ├── WaifuPicsSDK.ts        # Main SDK class
...
await image.load({ category: "example", type: "example" })
