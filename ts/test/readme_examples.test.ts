...
//     (\`new WaifuPicsSDK(...)\` / \`WaifuPicsSDK.test(...)\` become
//     \`WaifuPicsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"image":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new WaifuPicsSDK(...)\` and any
// \`WaifuPicsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WaifuPicsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WaifuPicsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
