...
class WaifuPicsSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = WaifuPicsHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = WaifuPicsHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!WaifuPicsFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, WaifuPicsFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return WaifuPicsUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = WaifuPicsHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = WaifuPicsHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = WaifuPicsHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new WaifuPicsError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = WaifuPicsHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new WaifuPicsError("graphql_error",
                "WaifuPicsSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
